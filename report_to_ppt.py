...
def main():
    # 设置命令行参数解析
    parser = argparse.ArgumentParser(description="Process a PDF and convert it to a PPT.")
    
    # 添加参数，并设置默认值
    parser.add_argument('pdf_path', type=str, nargs='?', default='检查报告.pdf', help='Path to the input PDF file')
    parser.add_argument('ppt_path', type=str, nargs='?', default='检查报告.pptx', help='Path to the output PPT file')

    args = parser.parse_args()

    # 运行 PDF 处理和 PPT 生成
    process_pdf_to_ppt(args.pdf_path, args.ppt_path)
